...
export default function ModulePage() {
  ...
  const doneCount = module.exercises.filter((e) => isExerciseComplete(e.id)).length;
  const allDone = doneCount === module.exercises.length;
      ...
      <header className="mt-4 mb-8">
        ...
        <p className="mt-2 text-sm">
          <span className={allDone ? 'text-term-green' : 'text-term-amber'}>
            {doneCount}/{module.exercises.length}
          </span>{' '}
          <span className="text-term-dim">exercises completed</span>
